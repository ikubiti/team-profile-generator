...
// Get the location weather forecast
...
let displayForecast = function (weatherData) {
	...
	let presentData = weatherData.list[0];
	let displayDate = dayjs.unix(presentData.dt).format('MM/DD/YYYY');
	...
	let mainDay = $(`<h3 class="font-boldest">${userParams[userCity]} (${displayDate}) <img src="${baseIcon}${presentData.weather[0].icon}.png" alt=${presentData.weather[0].description}></h3>`);
	currentCityData.append(mainDay);
	currentCityData.append($(`<p>Temp: ${presentData.main.temp}°F</p>`));
	currentCityData.append($(`<p>Wind: ${presentData.wind.speed} MPH</p>`));
	currentCityData.append($(`<p>Humidity: ${presentData.main.humidity}%</p>`));
